...
const RegisterForm = () => {
    ...
    const [successMsg, setSuccessMsg] = useState("");
        ...
        const verify = ()=>{
            ...
            if(reg.username.length<6 || reg.username.length>20 || !/^[A-Za-z0-9]+$/.test(reg.username)){
                isTrue=false;
                setErrors((prev)=>({...prev,username:"Username must be atleast 6 character  only numbers and letters!"}))
            }
        ...
        const Register = async (e) => {
            ...
            if(verify()){
                try{
                    const res = await axios.post("http://localhost:3001/register", {reg : reg});
