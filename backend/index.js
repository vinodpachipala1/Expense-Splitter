...
app.post("/CreateGroup", async (req, res) => {
    ...
    try{
        const rest= await db.query(`INSERT INTO groups (group_name, created_by) VALUES ($1, $2) RETURNING *`, [groupName, result.id]);
        const groupId = rest.rows[0].id;
        await db.query(`INSERT INTO group_members (group_id, user_id) VALUES($1,$2)`, [groupId, result.id]);
        res.send({"message": "Group Created Successfully!", id: rest.rows[0].id, name: rest.rows[0].group_name});
    } catch (err){
...
app.post("/getGroup", async (req, res) => {
    ...
    try{
        const data = await db.query(`SELECT id, group_name FROM groups WHERE created_by = $1 ORDER BY id ASC`, [result]);
...
app.post("/sendInvite", async (req, res) => {
    ...
    try{
        const result = await db.query(`SELECT id FROM users WHERE LOWER(email) = $1 OR LOWER(username) = $1`,[user]);
        ...
        if(users){
            ...
            const group_member = await db.query(`SELECT id FROM group_members WHERE group_id = $1 AND user_id = $2`,[groupId, userId]);
...
app.post("/getNotifications", async (req, res) => {
    ...
    try{
        const result = await db.query(`SELECT * FROM (SELECT notifications.user_id,notifications.id as notification_id, users.email AS sender_email, users.name as sender_name, notifications.message, notifications.group_id FROM users INNER JOIN notifications ON users.id = notifications.sent_from) AS MEOW WHERE USER_ID= $1`,[userId]);
...
app.post("/getGroupMembers", async (req,res) => {
    ...
    try{
        const result = await db.query(`SELECT * FROM (SELECT m.group_id, m.user_id, u.name, u.username FROM group_members m INNER JOIN groups g on m.group_id = g.id INNER JOIN users u on u.id = m.user_id) as members WHERE group_id = $1 ORDER BY name`, [id]);
...
app.post("/getActiveExpenses", async (req, res) => {
    ...
    try{
        const result = await db.query("SELECT * FROM (SELECT e.id,e.group_id, e.name, e.description, e.amount, e.amount, e.created_at, e.paid_by, u.name as paye_name FROM expenses e INNER JOIN users u ON e.paid_by = u.id) as expenses WHERE group_id = $1", [groupId]);
        const members = await db.query("SELECT * FROM (SELECT u.id as user_id, u.name, e.id as expense_id, e.group_id FROM expenses e INNER JOIN group_members m ON e.group_id = m.group_id INNER JOIN users u ON m.user_id = u.id) as expense_detail WHERE group_id = $1 ORDER BY name", [groupId]);
